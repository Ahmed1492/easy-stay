  ...
  return (
    ...
    >
      ...
      <div className="flex flex-col justify-center   sm:justify-between gap-2">
        ...
        {/*amenities  */}
        <div className="flex flex-wrap items-center  max-w-md  gap-3.5">
          {room.amenities.map((item, index) => (
            <div
              key={index}
              className="flex items-center gap-2 text-gray-500 text-sm  bg-slate-100 w-max py-2 px-4 rounded-lg  "
            >
              <img className="w-3  lg:w-4 " src={facilityIcons[item]} alt="" />
              <p className="font-semibold text-sm text-black">{item} </p>
            </div>
          ))}
        </div>
