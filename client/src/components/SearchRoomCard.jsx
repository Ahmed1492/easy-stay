...
const SearchRoomCard = ({ room, onQuickView }) => {
  const { currency } = useAppContext();
  const navigate = useNavigate();

  return (
    <div className="group bg-white rounded-2xl overflow-hidden border border-gray-100 shadow-sm hover:shadow-xl hover:border-gray-200 transition-all duration-300 flex flex-col">
      {/* Image */}
      ...
      <div className="p-4 flex flex-col flex-1">
        ...
        {/* Hotel name */}
        <h3 className="font-bold text-gray-900 text-base leading-snug mb-2 line-clamp-1 font-playfair">
          {room?.hotel?.name}
        </h3>
        ...
        <div className="flex items-center gap-1.5 mb-3">
          <div className="flex items-center gap-0.5">
            {[...Array(4)].map((_, i) => (
              <img key={i} src={assets.starIconFilled} alt="" className="w-3" />
            ))}
            <img src={assets.starIconOutlined} alt="" className="w-3" />
          </div>
